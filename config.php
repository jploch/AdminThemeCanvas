...
class AdminThemeUikitConfigHelper extends Wire {
	 ...
	 * 
	 */
	public function __construct(AdminThemeCanvas $adminTheme) {
	...
	public function configModule(InputfieldWrapper $inputfields) {
		...
		$f->notes = $this->_('Chose option B if you are having any trouble achieving intended Inputfield column widths.');
		...
		$f->notes = $defaultFileNote . " " .
			"[theme.css](" . $modules->wire('config')->urls('AdminThemeCanvas') . "theme/theme.css)";
		$f->icon = 'file-code-o';
		$fieldset2->add($f);
...
//      add field title hide option
		/** @var InputfieldRadios $f */
			$f = $modules->get('InputfieldCheckbox');
			$f->attr('name', 'hideFieldTitle');
            $f->type = 'checkbox';
			$f->label = $this->_('Hide Title');
            $f->checkboxLabel = $this->_('Hide Title');
			$f->icon = 'eye';
			if($field->get('hideFieldTitle')) $f->attr('checked', 'checked');
			$fieldset->add($f);
